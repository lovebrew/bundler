...
import { BundlerError, CompileError, ConvertError } from "../error";
...
export const processBundleGameFiles = async (zip: File): Promise<Blob> => {
  ...
  if ((await result.getContents()).length === 0) {
    throw new BundlerError(CompileError.FailedToCompile);
  }
...
export const processAssetGameFiles = async (
  ...
  if ((await container.getContents()).length === 0) {
    throw new BundlerError(ConvertError.FailedToConvertFiles);
  }

  const blob = await container.getBlob();
  return blob;
};
