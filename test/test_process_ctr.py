...
def test_build_ctr(client, version: int):
    """
    GIVEN a Flask application configured for testing
    WHEN a valid game is uploaded
    THEN check that the resulting zipfile contains a .3dsx file
    ...
    toml_file = modify_config_values(
        "build", [{"targets": ["ctr"], "app_version": version}]
    )
    ...
    with zipfile.ZipFile(io.BytesIO(response.data), "r") as archive:
        assert any("3dsx" in filename for filename in archive.namelist())
